...
class createpurchaseorder(models.TransientModel):
    ...
    def action_create_purchase_order(self):
        ...
        # post messages for creation
        message = f"Purchase Order created {purchase_order.name} at {purchase_order.date_order}"
        sale_order = self.env['sale.order'].browse(self._context.get('active_ids', []))
        sale_order.message_post(body=message)
        job_instance = self.env['jobcontrol.job'].browse(so_job_id)
        job_instance.message_post(body=message)
